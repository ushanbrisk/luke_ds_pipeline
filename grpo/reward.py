...
def accuracy_reward(completions: list[list[dict[str, str]]], solution: list[str], answer:list[str], **kwargs) -> list[Optional[float]]:
    """Reward function that checks if the completion is the same as the ground truth."""
    contents = [completion[0]["content"] for completion in completions]
    rewards = []

    sen_i = 0
    for content, sol, awr in zip(contents, solution, answer):
...
def accuracy_reward_bk(completions: list[list[dict[str, str]]], messages: list[str], **kwargs) -> list[Optional[float]]:
    """Reward function that checks if the completion is the same as the ground truth."""
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    for content, sol in zip(contents, messages):
        gold_parsed = parse(
            sol,
            extraction_mode="first_match",
        )
